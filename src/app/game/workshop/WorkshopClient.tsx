...
const UPGRADE_META: Record<string, { icon: string; label: string; description: string; baseCost: number; costScale: number; maxLevel: number; valueLabel: (v: number) => string }> = {
  develop_slots:       { icon: "M4 6h16M4 12h16M4 18h7", label: "Develop Slots", description: "Unlock additional simultaneous crafting slots.", baseCost: 2000, costScale: 2.5, maxLevel: 8,  valueLabel: (v) => `${2 + v} SLOTS` },
  develop_speed:       { icon: "M13 10V3L4 14h7v7l9-11h-7z", label: "Develop Speed", description: "Reduce part crafting time across all slots.", baseCost: 3000, costScale: 2.0, maxLevel: 10, valueLabel: (v) => `${v === 0 ? "0%" : `-${v * 5}%`}` },
  inventory_size:      { icon: "M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4", label: "Inventory Size (Parts)", description: "Increase maximum part storage capacity.", baseCost: 1000, costScale: 1.8, maxLevel: 18, valueLabel: (v) => `${20 + v * 10} PARTS` },
  inventory_mats_size: { icon: "M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4", label: "Inventory Size (Materials)", description: "Increase maximum material storage capacity.", baseCost: 800, costScale: 1.5, maxLevel: 18, valueLabel: (v) => `${200 + v * 100} MATS` },
  engineer_cap:        { icon: "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z", label: "Engineer Capacity", description: "Hire more engineers for your workshop.", baseCost: 2500, costScale: 2.0, maxLevel: 18, valueLabel: (v) => `${4 + v * 2} ENG` },
  driver_cap:          { icon: "M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z", label: "Driver Capacity", description: "Sign more drivers to your racing team.", baseCost: 2500, costScale: 2.0, maxLevel: 18, valueLabel: (v) => `${4 + v * 2} DRV` },
  garage_cap:          { icon: "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6", label: "Garage Capacity", description: "Store more cars in your facility.", baseCost: 1500, costScale: 1.6, maxLevel: 18, valueLabel: (v) => `${20 + v * 10} CARS` },
...
function upgradeCost(field: string, currentLevel: number): number {
  ...
  return Math.round(meta.baseCost * Math.pow(meta.costScale, currentLevel));
}
            ...
            <div className="animate-fade-up">
              <div className="flex items-center justify-between mb-4">
                <span style={{ fontFamily: "var(--font-mono)", fontSize: "11px", color: "var(--color-text-muted)" }}>
                  {data.engineers.length} / {getActualValue("engineer_cap", data.upgrades.engineer_cap)} ENGINEERS
                </span>
            ...
            <div className="animate-fade-up">
              ...
              <div className="mb-8">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: "11px", color: "var(--color-text-muted)" }}>
                    {data.inventory.length} / {getActualValue("inventory_size", data.upgrades.inventory_size)}
                  </span>
                </div>
