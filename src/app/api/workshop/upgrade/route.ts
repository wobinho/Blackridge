...
const UPGRADE_CONFIG: Record<string, { maxLevel: number; baseCost: number; costScale: number; label: string }> = {
  develop_slots:    { maxLevel: 6,  baseCost: 2000,  costScale: 2.5,  label: "Develop Slots" },
  develop_speed:    { maxLevel: 5,  baseCost: 3000,  costScale: 3.0,  label: "Develop Speed" },
  inventory_size:   { maxLevel: 8,  baseCost: 1000,  costScale: 1.8,  label: "Inventory Size" },
  engineer_cap:     { maxLevel: 10, baseCost: 2500,  costScale: 2.0,  label: "Engineer Capacity" },
  driver_cap:       { maxLevel: 15, baseCost: 2500,  costScale: 2.0,  label: "Driver Capacity" },
  garage_cap:       { maxLevel: 20, baseCost: 1500,  costScale: 1.6,  label: "Garage Capacity" },
...
export function upgradeCost(field: string, currentLevel: number): number {
  const cfg = UPGRADE_CONFIG[field];
  if (!cfg) return Infinity;
  return Math.round(cfg.baseCost * Math.pow(cfg.costScale, currentLevel - 1));
}
